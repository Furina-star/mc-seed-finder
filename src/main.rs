...
use mc_seed_finder::{
    find_confirmed_double, find_confirmed_quad, search, DOUBLE_HUT_CONDITION, QUAD_HUT_CONDITION,
};
...
fn main() {
    ...
    match cli.command {
        Command::Bench { count, mode } => {
            ...
            println!(
                "Benchmarking Pass-1 (geometry only, no biome filter) over {count} seeds [{mode:?}]..."
            );
            let elapsed_secs;
            let match_count;
            match mode {
                Mode::Double => {
                    let r =
                        search::scan_geometry(count, |seed| DOUBLE_HUT_CONDITION.evaluate(seed));
                    elapsed_secs = r.elapsed.as_secs_f64();
                    match_count = r.matches.len();
                    println!("Done in {:.2?}", r.elapsed);
                }
                Mode::Quad => {
                    let r = search::scan_geometry(count, |seed| QUAD_HUT_CONDITION.evaluate(seed));
                    elapsed_secs = r.elapsed.as_secs_f64();
                    match_count = r.matches.len();
                    println!("Done in {:.2?}", r.elapsed);
                }
